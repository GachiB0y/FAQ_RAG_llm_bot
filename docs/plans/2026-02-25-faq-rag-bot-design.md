...
```
# Документы
POST   /api/v1/admin/documents          # Добавить новый документ
GET    /api/v1/admin/documents          # Список всех документов
GET    /api/v1/admin/documents/{id}     # Детали документа
PUT    /api/v1/admin/documents/{id}     # Заменить документ (удаляет старые чанки)
DELETE /api/v1/admin/documents/{id}     # Удалить документ и его чанки
...
Каждый документ управляется независимо. При обновлении документа старая версия полностью удаляется из Qdrant, загружается новая. Это гарантирует, что бот не найдёт противоречивую информацию из разных версий.

### Сценарии использования

```
Текущее состояние:
├── Документ A (Правила IPSC 2024)
├── Документ B (Регламент)
└── Документ C (FAQ)

Задача: заменить A на новую версию, добавить D

Действия:
├── PUT /documents/{A.id} + новый файл → удаляет старые чанки A, создаёт новые
├── Документ B — не трогаем
├── Документ C — не трогаем
└── POST /documents + файл D → добавляет новый документ
```
...
|----------|----------------|
| **Добавить** (POST) | Парсинг → чанки → эмбеддинги → Qdrant + запись в PostgreSQL |
| **Заменить** (PUT) | Удаление старых чанков из Qdrant → парсинг нового файла → новые чанки |
| **Удалить** (DELETE) | Удаление чанков из Qdrant + удаление записи из PostgreSQL |
| **Список** (GET) | Показать все документы с метаданными (размер, дата, кол-во чанков) |
...
│  Документы                                    [+ Добавить]  │
├─────────────────────────────────────────────────────────────┤
│  ☑ Правила IPSC 2024.pdf      42 стр   Обновлён: 01.01.25  │
│                                        [Заменить] [Удалить] │
│                                                             │
│  ☑ Регламент соревнований.docx 15 стр  Обновлён: 15.12.24  │
│                                        [Заменить] [Удалить] │
│                                                             │
│  ☑ FAQ для участников.md       8 стр   Обновлён: 20.11.24  │
│                                        [Заменить] [Удалить] │
└─────────────────────────────────────────────────────────────┘
```

### Версионирование (не в MVP)

В будущем можно добавить хранение истории версий с возможностью отката. Для MVP достаточно полной замены.
