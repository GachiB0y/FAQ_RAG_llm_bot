...
> **Текущий шаг:** ещё не начали реализацию. Следующее действие — **Шаг A (uv)**,
> потом B (Makefile), потом F (Prefect). Идём по порядку — фундамент сначала.
...
- 2026-07-06: создан план + вводные доки (uv, Makefile, Prefect intro). Решили
  идти по порядку A→B→F. Реализация ещё не начата.

---

## Порядок шагов (по нарастающей)

### ⬜ Шаг A — `uv` + lockfile (лечит боль #5 «бардак в окружении»)

📖 **Вводная перед реализацией:** [uv-intro](2026-07-06-uv-intro.md)

Заменить хаотичный `pip install` на `uv` с зафиксированным lock-файлом.

- [ ] Установить `uv` в backend-образ (или локально)
- [ ] Сгенерировать `uv.lock` из текущих зависимостей (включая ragas==0.2.15,
      langchain<0.4 и прочие пины что мы вручную подбирали)
- [ ] Зафиксировать eval-зависимости отдельной группой (`ragas`, `mlflow`,
      `fastembed`, `tiktoken`, `matplotlib`, `pymupdf`) — сейчас они ставились
      руками через `pip install` в работающий контейнер
- [ ] Проверить: чистая пересборка контейнера → всё работает без ручного доустановки
- [ ] Обновить Dockerfile на `uv sync`

**Готово когда:** `docker compose build backend` даёт рабочее окружение со всеми
eval-зависимостями без единого ручного `pip install`.
