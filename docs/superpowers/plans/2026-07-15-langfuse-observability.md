...
### Task 5: Custom-прайсинг (A3.2), verify (A3.3), обновление статуса ✅ ВЫПОЛНЕНО 2026-07-16

> **Факты (2026-07-16):** прогон 3 вопросов (subset, свежий, LANGFUSE_ENABLED=true) на
> генераторе `qwen/qwen3.6-plus` + судья `openai/gpt-5.4`. Трейсы генератора с ненулевым
> cost + по 4 Ragas-Scores; трейс судьи. Сверка A3.3: Langfuse ~$0.184 vs OpenRouter delta
> $0.177 (~4%, custom-цены vs факт-биллинг) → доверяем. Судья = ~95% cost подтверждён.
> Пред-полётные отловы (без траты денег): qwen/qwen3.6 не существует на OR → -plus;
> `llama-index-llms-openai-like` отсутствовал в lock (боль #5); Ollama-контейнер был
> остановлен → первый прогон дал нули (генерация падала), после старта Ollama — норм.
> Статусы обновлены (PROJECT_STATUS, mlops-tracker).
