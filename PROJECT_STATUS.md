...
**E1 (Telegram-бот) ✅ код готов** (2026-07-19, SDD/TDD, ветка `dev`) — осталось живьём (токен @BotFather + пересоздать backend + `OPENROUTER_API_KEY` в `.env`).
**Следующее действие: живой прогон E1 + E2 (презентация, +B4-отчёт)** либо E5 (кэш). Опц.: B4 Этап 2 (`top_k`-свип).
...
- [x] **E1 — код готов ✅ 2026-07-19** Telegram-бот (aiogram 3, polling) — отдельный сервис
      `bot/`, тонкий HTTP-клиент `/api/v1/chat`. Логинится служебным bot-юзером (role=user,
      `make bot-seed`), шлёт `X-Telegram-User-Id` → `chat.py` через `resolve_actor_id` берёт
      `tg:<id>` для rate-limit (E4) и Langfuse (A3) **только** от bot-юзера (веб-контур не тронут).
      Показ квоты: заголовки `X-RateLimit-*` → бот пишет «Осталось N из 10» + «через Xч Yм» на 429.
      SDD/TDD: 9 задач + фикс + финальное ревью (opus) — все чисто; backend 70 passed, bot 21 passed.
      **Осталось живьём:** токен @BotFather → `.env`, `docker compose up -d --force-recreate backend`
      (подхватить `TELEGRAM_BOT_USER_EMAIL`), `OPENROUTER_API_KEY` в `.env`, `make bot-up`.
      Спека/план: [design](docs/superpowers/specs/2026-07-19-e1-telegram-bot-design.md) ·
      [plan](docs/superpowers/plans/2026-07-19-e1-telegram-bot.md).
...
| 19.07 | **E1 (Telegram-бот) — код готов ✅** (SDD/TDD: спека→план→9 задач+фикс+финальное ревью opus, всё чисто). Новый сервис `bot/` (aiogram 3, polling) — тонкий клиент `/api/v1/chat`. Backend-правки: `resolve_actor_id` (tg-namespace идентификатор только от bot-юзера → per-tg rate-limit E4 + Langfuse, веб-контур не тронут), остаток квоты в заголовках `X-RateLimit-*` (лимитер `hit`+`RateLimitStatus`, `ChatResponse` не менялся), сид bot-юзера `seed_bot.py`+`make bot-seed`. Бот: login+перелогин на 401, форматирование ответа/источников/квоты, `/start`. Инфра: `telegram-bot` в compose, `make bot-up/bot-logs`, `.env.example`. Тесты: backend 70 passed, bot 21 passed. Живой прогон отложен (токен @BotFather + пересоздать backend + `OPENROUTER_API_KEY`; платную генерацию не запускали — pre-flight). | [spec](docs/superpowers/specs/2026-07-19-e1-telegram-bot-design.md), [план](docs/superpowers/plans/2026-07-19-e1-telegram-bot.md) |
